...
#root.geometry("%dx%d" %(screen_width, screen_height)) #Full screen
...
def barecode():
    ...
    global sql_con
    sql_con = cur.execute("SELECT id FROM main_table")
    global result
    result = cur.fetchall()
    ...
    #################################################################
    with open('backup_txt/%s.txt' % ID_name, 'w', encoding='utf-8') as f:
        f.write('ID : ' + ID + '\t')
